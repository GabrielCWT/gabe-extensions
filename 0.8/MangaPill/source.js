...
class MangaPill {
    constructor(cheerio) {
        ...
        this.RETRY = 5;
        this.parser = new MangaPillParser_1.Parser();
    }
    ...
    async getSearchResults(query, metadata) {
        let searchParams = '';
        searchParams = searchParams.concat(`q=${query.title ?? ''}`);
        for (const tag of query.includedTags) {
            searchParams = searchParams.concat(`&genre=${tag.id}`);
        }
        const request = App.createRequest({
...
},{"./MangaPillParser":71,"@paperback/types":61}],71:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
class Parser {
    parseMangaDetails($, mangaId) {
        ...
        const description = this.decodeHTMLEntity($('.text-sm.text--secondary').text().trim());
    ...
    parseChapterDetails($, mangaId, id) {
        ...
        for (const pageObj of $('picture > img').get()) {
            const page = $(pageObj).attr('data-src') ?? $(pageObj).attr('src') ?? '';
